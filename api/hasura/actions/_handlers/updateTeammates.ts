...
import { getMemberFromProfileIdWithCircle } from '../../../../api-lib/nominees';
...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const user = await getMemberFromProfileIdWithCircle(profileId, circle_id);
  if (!user) {
  ...
  const insertInput = teammatesToKeep.map(t => {
    return { member_id: user.id, team_mate_id: t };
  });
      ...
      insert_teammates: [
        {
          ...
          on_conflict: {
            constraint:
              teammates_constraint.teammates_member_id_team_mate_id_key,
            update_columns: [],
          },
        },
        {
  ...
  return res.status(200).json({ member_id: user.id });
}
