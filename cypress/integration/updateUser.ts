...
context('Coordinape', () => {
  ...
  it("can update user's address and restore it to previous value", () => {
    ...
    const newAddress = '0x51508887c3fef0b4390091c5a4b2b91562881526'.toLowerCase();
    ...
    // Click on edit user
    cy.contains('Alda',{timeout:120000})
        .parents('tr')
        .within(()=>{
          cy.get('td').last().get('[d^=M4]')
              .click(); // This selects the pencil SVG
        });



    // enter the new address
    cy.get('[value='+oldAddress+']').clear().click().type(newAddress);
    ...
    cy.get('[value='+newAddress+']').clear().click().type(oldAddress);
...
const assertAddr = (addr: string) => {
  cy.contains('Alda',{timeout:120000})
      .parents('tr')
      .within(()=>{
        cy.get('td').eq(1).contains(addr.substr(0,6));
      });
};