...
usage() {
  ...
  echo "Go look in ${SEED_DIR} for files that match the pattern '*.csv' and choose the timestamp where you want to start e.g. '2024-06-25-105415'."
  exit 1
}
...
get_table_name() {
    local file_name="$1"
    local table_name=$(echo "$file_name" | sed -E 's/.*\.public\.(.+)\.csv/\1/')
    echo "$table_name"
}
...
do
    ...
    then
        ...
        if [ "$truncate_set" = true ]; then
            echo
            echo "Truncating table public.$table_name"
            command="psql postgres://postgres@localhost:5432/postgres -c \"TRUNCATE TABLE public.$table_name;\""
            echo "Executing: $command"
            eval "$command"
        fi

        # Construct and execute the COPY command
        command="psql postgres://postgres@localhost:5432/postgres -c \"\COPY public.$table_name FROM '$file' CSV HEADER;\""

        echo "Executing: $command"
        eval "$command"

        echo "Finished copying data from $file to $table_name"
        echo
