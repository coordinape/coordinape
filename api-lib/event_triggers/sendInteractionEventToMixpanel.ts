...
type Event = Omit<
  EventTriggerPayload<'interaction_events', 'INSERT'>['event']['data']['new'],
  '__typename'
>;
...
export const addPropsAndTrack = async (
  ...
  if (event.circle_id && trackCircleEvent) {
    await track(mp, `circle_${event.event_type}`, `circle_${event.circle_id}`, {
      profile_id: event.profile_id ? sha256(`${event.profile_id}`) : undefined,
      ...eventData,
    });
  }
};
