...
import { getMemberByIdAndCurrentEpoch } from '../gql/queries';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const {
    event: { data },
  }: EventTriggerPayload<'members', 'UPDATE'> = req.body;
  ...
  const results = [];
  try {
    const member = await getMemberByIdAndCurrentEpoch(id, circle_id);
    assert(member, 'panic: member must exist');

    const { pending_sent_gifts, pending_received_gifts, id: memberId } = member;
