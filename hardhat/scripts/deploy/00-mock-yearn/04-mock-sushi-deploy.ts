...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const sushi = await deploy('SUSHI', {
    contract: 'MockToken',
    from: deployer,
    args: ['SushiToken', 'SUSHI'],
    log: true,
  });
  ...
  const yvSUSHI = await deploy('yvSUSHI', {
    contract: 'MockVault',
    from: deployer,
    args: [sushi.address, 'SUSHI yVault', 'yvSUSHI'],
  });

  await yRegistry.addVault(sushi.address, yvSUSHI.address);
  return !useProxy;
};
export default func;
func.id = 'deploy_mock_yvsushi';
func.tags = ['DeployMockYvSUSHI', 'MockYvSUSHI'];
