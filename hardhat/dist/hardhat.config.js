...
(0, config_1.task)(
  ...
  async (args, hre) => {
    ...
    accounts.forEach(async (account, i) => {
      ...
      console.log(
        `(${accountId}) ${account.address} (${ethers_1.ethers.utils.formatEther(
          balance
        )} ETH)`
      );
    });
...
(0, config_1.task)('wrap', 'Wraps the given amount of ETH to WETH')
  ...
  .setAction(async (args, hre) => {
    const sender = await (0, unlockSigner_1.unlockSigner)(
      constants_1.HARDHAT_OWNER_ADDRESS,
      hre
    );
    const weth = new ethers_1.ethers.Contract(
      tokens.WETH.addr,
    ...
    console.log(
      `Wrapped ${args.amount} ETH for ${constants_1.HARDHAT_OWNER_ADDRESS}`
    );
  });
(0, config_1.task)('unwrap', 'Unwraps the given amount of WETH to ETH')
  ...
  .setAction(async (args, hre) => {
    ...
    console.log(
      `Unwrapped ${args.amount} WETH for ${constants_1.HARDHAT_OWNER_ADDRESS}`
    );
  });
