...
async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  // This is to workaround module import errors
  import('@magic-sdk/admin')
    .then(async magicModule => {
      const Magic = magicModule.Magic;

      // Magic.assert(API_KEY, 'MAGIC_SECRET_API_KEY is missing');
      ...
      try {
        ...
        if (e instanceof Error)
          console.error(`Failed to fetch users emails` + e.message);
    ...
    .catch(err => {
      console.error('Error importing @magic-sdk/admin:', err);
    });
}
